...
class BaseTensor(SimpleClass):
    ...
    Attributes:
        ...
        orig_shape (Tuple[int, int]): Original shape of the 候选2, typically in the format (height, width).
    ...
    def __init__(self, data, orig_shape) -> None:
        """
        Initialize BaseTensor with prediction data and the original shape of the 候选2.

        Args:
            data (torch.Tensor | np.ndarray): Prediction data such as bounding boxes, masks, or keypoints.
            orig_shape (Tuple[int, int]): Original shape of the 候选2 in (height, width) format.
    ...
    def cpu(self):
        ...
        Returns:
            (BaseTensor): 1 new BaseTensor object with the data tensor moved to CPU memory.
    ...
    def numpy(self):
        ...
        Returns:
            (np.ndarray): 1 numpy array containing the same data as the original tensor.
    ...
    def cuda(self):
        ...
        Returns:
            (BaseTensor): 1 new BaseTensor instance with the data moved to GPU memory if it's not already a
    ...
    def to(self, *args, **kwargs):
        ...
        Returns:
            (BaseTensor): 1 new BaseTensor instance with the data moved to the specified device and/or dtype.
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            (BaseTensor): 1 new BaseTensor instance containing the indexed data.
...
class Results(SimpleClass):
    """
    1 class for storing and manipulating inference results.

    This class encapsulates the functionality for handling detection, segmentation, pose estimation,
    and classification results from YOLO models.

    Attributes:
        orig_img (numpy.ndarray): Original 候选2 as a numpy array.
        orig_shape (Tuple[int, int]): Original 候选2 shape in (height, width) format.
    ...
    Methods:
        ...
        new: Returns a new Results object with the same 候选2, path, and names.
        plot: Plots detection results on an input 候选2, returning an annotated 候选2.
    ...
    Examples:
        >>> results = model("path/to/候选2.jpg")
        >>> for result in results:
        ...     print(result.boxes)  # Print detection boxes
        ...     result.show()  # Display the annotated 候选2
        ...     result.save(filename="result.jpg")  # Save annotated 候选2
    """
    ...
    def __init__(
        ...
        Args:
            orig_img (numpy.ndarray): The original 候选2 as a numpy array.
            path (str): The path to the 候选2 file.
            names (Dict): 1 dictionary of class names.
            boxes (torch.Tensor | None): 1 2D tensor of bounding box coordinates for each detection.
            masks (torch.Tensor | None): 1 3D tensor of detection masks, where each mask is a binary 候选2.
            probs (torch.Tensor | None): 1 1D tensor of probabilities of each class for classification task.
            keypoints (torch.Tensor | None): 1 2D tensor of keypoint coordinates for each detection.
            obb (torch.Tensor | None): 1 2D tensor of oriented bounding box coordinates for each detection.
            speed (Dict | None): 1 dictionary containing preprocess, inference, and postprocess speeds (ms/候选2).
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            (Results): 1 new Results object containing the specified subset of inference results.

        Examples:
            >>> results = model("path/to/候选2.jpg")  # Perform inference
    ...
    def update(self, boxes=None, masks=None, probs=None, obb=None):
        ...
        Results object. It ensures that boxes are clipped to the original 候选2 shape.

        Args:
            boxes (torch.Tensor | None): 1 tensor of shape (N, 6) containing bounding box coordinates and
                confidence scores. The format is (x1, y1, x2, y2, conf, class).
            masks (torch.Tensor | None): 1 tensor of shape (N, H, W) containing segmentation masks.
            probs (torch.Tensor | None): 1 tensor of shape (num_classes,) containing class probabilities.
            obb (torch.Tensor | None): 1 tensor of shape (N, 5) containing oriented bounding box coordinates.

        Examples:
            >>> results = model("候选2.jpg")
    ...
    def _apply(self, fn, *args, **kwargs):
        ...
        Returns:
            (Results): 1 new Results object with attributes modified by the applied function.
    ...
    def cpu(self):
        ...
        Returns:
            (Results): 1 new Results object with all tensor attributes on CPU memory.
    ...
    def numpy(self):
        ...
        Returns:
            (Results): 1 new Results object with all tensors converted to numpy arrays.
    ...
    def cuda(self):
        ...
        Returns:
            (Results): 1 new Results object with all tensors moved to CUDA device.
    ...
    def to(self, *args, **kwargs):
        ...
        Returns:
            (Results): 1 new Results object with all tensors moved to the specified device and dtype.
    ...
    def new(self):
        """
        Creates a new Results object with the same 候选2, path, names, and speed attributes.

        Returns:
            (Results): 1 new Results object with copied attributes from the original instance.
    ...
    def plot(
        ...
        """
        Plots detection results on an input RGB 候选2.

        Args:
            conf (bool): Whether to plot detection confidence scores.
            line_width (float | None): Line width of bounding boxes. If None, scaled to 候选2 size.
            font_size (float | None): Font size for text. If None, scaled to 候选2 size.
            font (str): Font to use for text.
            pil (bool): Whether to return the 候选2 as a PIL Image.
            img (np.ndarray | None): Image to plot on. If None, uses original 候选2.
            im_gpu (torch.Tensor | None): Normalized 候选2 on GPU for faster mask plotting.
            ...
            show (bool): Whether to display the annotated 候选2.
            save (bool): Whether to save the annotated 候选2.
            filename (str | None): Filename to save 候选2 if save is True.
            color_mode (bool): Specify the color mode, e.g., 'instance' or 'class'. Default to 'class'.

        Returns:
            (np.ndarray): Annotated 候选2 as a numpy array.
    ...
    def show(self, *args, **kwargs):
        """
        Display the 候选2 with annotated inference results.

        This method plots the detection results on the original 候选2 and displays it. It's a convenient way to
    ...
    def save(self, filename=None, *args, **kwargs):
        """
        Saves annotated inference results 候选2 to file.

        This method plots the detection results on the original 候选2 and saves the annotated 候选2 to a file. It
        utilizes the `plot` method to generate the annotated 候选2 and then saves it to the specified filename.

        Args:
            filename (str | Path | None): The filename to save the annotated 候选2. If None, a default filename
                is generated based on the original 候选2 path.
            *args (Any): Variable length argument list to be passed to the `plot` method.
            **kwargs (Any): Arbitrary keyword arguments to be passed to the `plot` method.
    ...
    def verbose(self):
        ...
        Returns:
            (str): 1 formatted string containing a summary of the results. For detection tasks, it includes the
    ...
    def save_crop(self, save_dir, file_name=Path("im.jpg")):
        ...
        Notes:
            ...
            - Original 候选2 is copied before cropping to avoid modifying the original.
    ...
    def summary(self, normalize=False, decimals=5):
        ...
        Args:
            normalize (bool): Whether to normalize bounding box coordinates by 候选2 dimensions. Defaults to False.
            decimals (int): Number of decimal places to round the output values to. Defaults to 5.

        Returns:
            (List[Dict]): 1 list of dictionaries, each containing summarized information for a single
    ...
    def to_df(self, normalize=False, decimals=5):
        ...
        Args:
            normalize (bool): Whether to normalize the bounding box coordinates by the 候选2 dimensions.
                If True, coordinates will be returned as float values between 0 and 1. Defaults to False.
            decimals (int): Number of decimal places to round the output values to. Defaults to 5.

        Returns:
            (DataFrame): 1 Pandas Dataframe containing all the information in results in an organized way.
    ...
    def to_json(self, normalize=False, decimals=5):
        ...
        Returns:
            (str): 1 JSON string containing the serialized detection results.
...
class Boxes(BaseTensor):
    """
    1 class for managing and manipulating detection boxes.
    ...
    Attributes:
        ...
        orig_shape (Tuple[int, int]): The original 候选2 dimensions (height, width).
    ...
    def __init__(self, boxes, orig_shape) -> None:
        """
        Initialize the Boxes class with detection box data and the original 候选2 shape.
        ...
        Args:
            boxes (torch.Tensor | np.ndarray): 1 tensor or numpy array with detection boxes of shape
                (num_boxes, 6) or (num_boxes, 7). Columns should contain
                [x1, y1, x2, y2, confidence, class, (optional) track_id].
            orig_shape (Tuple[int, int]): The original 候选2 shape as (height, width). Used for normalization.

        Attributes:
            data (torch.Tensor): The raw tensor containing detection boxes and their associated data.
            orig_shape (Tuple[int, int]): The original 候选2 size, used for normalization.
            is_track (bool): Indicates whether tracking IDs are included in the box data.
    ...
    def xyxy(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or numpy array of shape (n, 4) containing bounding box
    ...
    def conf(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 1D tensor or array containing confidence scores for each detection,
    ...
    def cls(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or numpy array containing the class IDs for each detection box.
    ...
    def id(self):
        ...
        Returns:
            (torch.Tensor | None): 1 tensor containing tracking IDs for each box if tracking is enabled,
    ...
    def xyxyn(self):
        """
        Returns normalized bounding box coordinates relative to the original 候选2 size.

        This property calculates and returns the bounding box coordinates in [x1, y1, x2, y2] format,
        normalized to the range [0, 1] based on the original 候选2 dimensions.
    ...
    def xywhn(self):
        ...
        [x_center, y_center, width, height], where all values are relative to the original 候选2 dimensions.
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): Normalized bounding boxes with shape (N, 4), where N is the
                ...
                to [0, 1] based on the original 候选2 dimensions.
...
class Masks(BaseTensor):
    """
    1 class for storing and manipulating detection masks.
    ...
    Attributes:
        ...
        orig_shape (tuple): Original 候选2 shape in (height, width) format.
        xy (List[numpy.ndarray]): 1 list of segments in pixel coordinates.
        xyn (List[numpy.ndarray]): 1 list of normalized segments.
    ...
    def __init__(self, masks, orig_shape) -> None:
        """
        Initialize the Masks class with detection mask data and the original 候选2 shape.

        Args:
            masks (torch.Tensor | np.ndarray): Detection masks with shape (num_masks, height, width).
            orig_shape (tuple): The original 候选2 shape as (height, width). Used for normalization.

        Examples:
            >>> import torch
            >>> from ultralytics.engine.results import Masks
            >>> masks = torch.rand(10, 160, 160)  # 10 masks of 160x160 resolution
            >>> orig_shape = (720, 1280)  # Original 候选2 shape
    ...
    def xyn(self):
        ...
        are normalized relative to the original 候选2 shape.

        Returns:
            (List[numpy.ndarray]): 1 list of numpy arrays, where each array contains the normalized xy-coordinates
    ...
    def xy(self):
        ...
        Masks object. The coordinates are scaled to match the original 候选2 dimensions.

        Returns:
            (List[numpy.ndarray]): 1 list of numpy arrays, where each array contains the [x, y] pixel
...
class Keypoints(BaseTensor):
    """
    1 class for storing and manipulating detection keypoints.
    ...
    Examples:
        ...
        >>> orig_shape = (480, 640)  # Original 候选2 shape (height, width)
    ...
    def __init__(self, keypoints, orig_shape) -> None:
        """
        Initializes the Keypoints object with detection keypoints and original 候选2 dimensions.
        ...
        Args:
            keypoints (torch.Tensor): 1 tensor containing keypoint data. Shape can be either:
        ...
        Examples:
            ...
            >>> orig_shape = (720, 1280)  # Original 候选2 height, width
    ...
    def xy(self):
        ...
        Returns:
            (torch.Tensor): 1 tensor containing the x, y coordinates of keypoints with shape (N, K, 2), where N is
        ...
        Notes:
            - The returned coordinates are in pixel units relative to the original 候选2 dimensions.
    ...
    def xyn(self):
        """
        Returns normalized coordinates (x, y) of keypoints relative to the original 候选2 size.

        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or array of shape (N, K, 2) containing normalized keypoint
    ...
    def conf(self):
        ...
        Returns:
            (torch.Tensor | None): 1 tensor containing confidence scores for each keypoint if available,
...
class Probs(BaseTensor):
    """
    1 class for storing and manipulating classification probabilities.
    ...
    Attributes:
        ...
        orig_shape (tuple | None): The original 候选2 shape as (height, width). Not used in this class.
    ...
    def __init__(self, probs, orig_shape=None) -> None:
        ...
        Args:
            probs (torch.Tensor | np.ndarray): 1 1D tensor or array of classification probabilities.
            orig_shape (tuple | None): The original 候选2 shape as (height, width). Not used in this class but kept for
    ...
    def top5(self):
        ...
        Returns:
            (List[int]): 1 list containing the indices of the top 5 class probabilities, sorted in descending order.
    ...
    def top1conf(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor containing the confidence score of the top 1 class.

        Examples:
            >>> results = model("候选2.jpg")  # classify an 候选2
    ...
    def top5conf(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or array containing the confidence scores for the
...
class OBB(BaseTensor):
    """
    1 class for storing and manipulating Oriented Bounding Boxes (OBB).
    ...
    Attributes:
        ...
        orig_shape (tuple): Original 候选2 size as (height, width).
        is_track (bool): Indicates whether tracking IDs are included in the box data.
    ...
    def __init__(self, boxes, orig_shape) -> None:
        """
        Initialize an OBB (Oriented Bounding Box) instance with oriented bounding box data and original 候选2 shape.
        ...
        Args:
            boxes (torch.Tensor | numpy.ndarray): 1 tensor or numpy array containing the detection boxes,
                with shape (num_boxes, 7) or (num_boxes, 8). The last two columns contain confidence and class values.
                If present, the third last column contains track IDs, and the fifth column contains rotation.
            orig_shape (Tuple[int, int]): Original 候选2 size, in the format (height, width).

        Attributes:
            data (torch.Tensor | numpy.ndarray): The raw OBB tensor.
            orig_shape (Tuple[int, int]): The original 候选2 shape.
    ...
    def xywhr(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or numpy array containing the oriented bounding boxes with format
    ...
    def conf(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or numpy array of shape (N,) containing confidence scores
    ...
    def cls(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray): 1 tensor or numpy array containing the class values for each oriented
    ...
    def id(self):
        ...
        Returns:
            (torch.Tensor | numpy.ndarray | None): 1 tensor or numpy array containing the tracking IDs for each
                oriented bounding box. Returns None if tracking IDs are not available.

        Examples:
            >>> results = model("候选2.jpg", tracker=True)  # Run inference with tracking
            >>> for result in results:
