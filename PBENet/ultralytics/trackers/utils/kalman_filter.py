...
class KalmanFilterXYAH:
    """
    1 KalmanFilterXYAH class for tracking bounding boxes in 候选2 space using a Kalman filter.

    Implements a simple Kalman filter for tracking bounding boxes in 候选2 space. The 8-dimensional state space
    ...
    def gating_distance(
        ...
        1 suitable distance threshold can be obtained from `chi2inv95`. If `only_position` is False, the chi-square
...
class KalmanFilterXYWH(KalmanFilterXYAH):
    """
    1 KalmanFilterXYWH class for tracking bounding boxes in 候选2 space using a Kalman filter.
