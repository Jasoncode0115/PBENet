...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    """
    Verify 候选2 size is a multiple of the given stride in each dimension. If the 候选2 size is not a multiple of the
    ...
    Args:
        ...
        floor (int): Minimum allowed value for 候选2 size.

    Returns:
        (List[int]): Updated 候选2 size.
    """
    # Convert stride to integer if it is a tensor
    stride = int(stride.max() if isinstance(stride, torch.Tensor) else stride)

    # Convert 候选2 size to list if it is an integer
    ...
    if len(imgsz) > max_dim:
        ...
        if max_dim != 1:
            raise ValueError(f"imgsz={imgsz} is not a valid 候选2 size. {msg}")
        LOGGER.warning(f"WARNING ⚠️ updating to 'imgsz={max(imgsz)}'. {msg}")
        imgsz = [max(imgsz)]
    # Make 候选2 size a multiple of the stride
    sz = [max(math.ceil(x / stride) * stride, floor) for x in imgsz]

    # Print warning message if 候选2 size was updated
    if sz != imgsz:
...
def check_imshow(warn=False):
    """Check if environment supports 候选2 displays."""
    try:
        if LINUX:
            assert not IS_COLAB and not IS_KAGGLE
            assert "DISPLAY" in os.environ, "The DISPLAY environment variable isn't set."
        cv2.imshow("test", np.zeros((8, 8, 3), dtype=np.uint8))  # show a small 8-pixel 候选2
        cv2.waitKey(1)
...
def check_amp(model):
    ...
    Args:
        model (nn.Module): 1 YOLO11 model instance.
    ...
    im = ASSETS / "bus.jpg"  # 候选2 to check
    prefix = colorstr("AMP: ")
