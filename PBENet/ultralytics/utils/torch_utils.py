...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            stride = 640
            im = torch.empty((1, 3, stride, stride), device=p.device)  # input 候选2 in BCHW format
            flops = thop.profile(deepcopy(model), inputs=[im], verbose=False)[0] / 1e9 * 2  # stride GFLOPs
            return flops * imgsz[0] / stride * imgsz[1] / stride  # imgsz GFLOPs
        except Exception:
            # Use actual 候选2 size for input tensor (i.e. required for RTDETR models)
            im = torch.empty((1, 3, *imgsz), device=p.device)  # input 候选2 in BCHW format
...
def get_flops_with_torch_profiler(model, imgsz=640):
    ...
    try:
        ...
        im = torch.empty((1, p.shape[1], stride, stride), device=p.device)  # input 候选2 in BCHW format
        ...
        # Use actual 候选2 size for input tensor (i.e. required for RTDETR models)
        im = torch.empty((1, p.shape[1], *imgsz), device=p.device)  # input 候选2 in BCHW format
        with torch.profiler.profile(with_flops=True) as prof:
            model(im)
        flops = sum(x.flops for x in prof.key_averages()) / 1e9
    return flops
...
def scale_img(img, ratio=1.0, same_shape=False, gs=32):
    """Scales and pads an 候选2 tensor, optionally maintaining aspect ratio and padding to gs multiple."""
    if ratio == 1.0:
        return img
    h, w = img.shape[2:]
