...
class v8DetectionLoss:
    ...
    def compute_loss(self, preds, batch):
        ...
        imgsz = torch.tensor(feats[0].shape[2:], device=self.device, dtype=dtype) * self.stride[0]  # 候选2 size (h,w)
...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def single_mask_loss(
        ...
        """
        Compute the instance segmentation loss for a single 候选2.
        ...
        Returns:
            (torch.Tensor): The calculated mask loss for a single 候选2.
    ...
    def calculate_segmentation_loss(
        ...
        Args:
            fg_mask (torch.Tensor): 1 binary tensor of shape (BS, N_anchors) indicating which anchors are positive.
            ...
            imgsz (torch.Tensor): Size of the input 候选2 as a tensor of shape (2), i.e., (H, W).
...
class v8PoseLoss(v8DetectionLoss):
    ...
    def kpts_decode(anchor_points, pred_kpts):
        """Decodes predicted keypoints to 候选2 coordinates."""
        y = pred_kpts.clone()
        y[..., :2] *= 2.0
    ...
    def calculate_keypoints_loss(
        ...
        # Find the maximum number of keypoints in a single 候选2
