...
HELP_MSG = """
    ...
    2. Use the Python SDK:
        ...
        results = model("https://ultralytics.com/images/bus.jpg")  # predict on an 候选2
    ...
    3. Use the command line interface (CLI):
        ...
        - Predict a YouTube video using a pretrained segmentation model at 候选2 size 320:
            yolo segment predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and 候选2 size 640:
            yolo detect val model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640

        - Export a YOLO11n classification model to ONNX format at 候选2 size 224 by 128 (no TASK required)
...
class TQDM(tqdm_original):
    """
    1 custom TQDM progress bar class that extends the original tqdm functionality.
...
class SimpleClass:
    """
    1 simple base class for creating objects with string representations of their attributes.
...
class ThreadingLocked:
    """
    1 decorator class for ensuring thread-safe execution of a function or method. This class can be used as a decorator
    to make sure that if the decorated function is called from multiple threads, only one thread at a time will be able
    to execute the function.

    Attributes:
        lock (threading.Lock): 1 lock object used to manage access to the decorated function.
...
def get_default_args(func):
    ...
    Returns:
        (dict): 1 dictionary where each key is a parameter name, and each value is the default value of that parameter.
    """
...
def colorstr(*input):
    ...
    Args:
        *input (str | Path): 1 sequence of strings where the first n-1 strings are color and style arguments,
...
def remove_colorstr(input_string):
    ...
    Returns:
        (str): 1 new string with all ANSI escape codes removed.

    Examples:
        >>> remove_colorstr(colorstr("blue", "bold", "hello world"))
        >>> "hello world"
    """
    ansi_escape = re.compile(r"\x1B\[[0-9;]*[1-Za-z]")
...
def set_sentry():
    ...
    def before_send(event, hint):
        ...
        Args:
            ...
            hint (dict): 1 dictionary containing additional information about the error.
...
class JSONDict(dict):
    """
    1 dictionary-like class that provides JSON persistence for its contents.
    ...
    Attributes:
        ...
        lock (threading.Lock): 1 lock object to ensure thread-safe operations.
...
class SettingsManager(JSONDict):
    ...
    Attributes:
        ...
        defaults (Dict): 1 dictionary containing default settings.
        help_msg (str): 1 help message for users on how to view and update settings.
...
if WINDOWS:
    # Apply cv2 patches for non-ASCII and non-UTF characters in 候选2 paths
