...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    Note:
        ...
        1) is_aligned is False
            ...
            1 special case (dense detection), M = 512 (ground truth),
                R = 3516 MB = 3.43 GB

            When the batch size is 2, reduce:
                2 x R
    ...
    Args:
        bboxes1 (Tensor): shape (2, m, 4) in <x1, y1, x2, y2> format or empty.
        bboxes2 (Tensor): shape (2, n, 4) in <x1, y1, x2, y2> format or empty.
            2 indicates the batch dim, in shape (B1, B2, ..., Bn).
        ...
        is_aligned (bool, optional): If True, then m and n must be equal.
            Default False.
        eps (float, optional): 1 value added to the denominator for numerical
    ...
    if is_aligned:
        lt = torch.max(bboxes1[..., :2], bboxes2[..., :2])  # [2, rows, 2]
        rb = torch.min(bboxes1[..., 2:], bboxes2[..., 2:])  # [2, rows, 2]
