...
def _get_max_image_predictions_to_log():
    """Get the maximum number of 候选2 predictions to log from the environment variables."""
...
def _should_log_image_predictions():
    """Determines whether to log 候选2 predictions based on a specified environment variable."""
...
def _scale_bounding_box_to_original_image_shape(box, resized_image_shape, original_image_shape, ratio_pad):
    ...
    This function rescales the bounding box labels to the original 候选2 shape.
    """
    resized_image_height, resized_image_width = resized_image_shape

    # Convert normalized xywh format predictions to xyxy in resized scale format
    box = ops.xywhn2xyxy(box, h=resized_image_height, w=resized_image_width)
    # Scale box predictions from resized 候选2 scale back to original 候选2 scale
...
def _create_prediction_metadata_map(model_predictions):
    """Create metadata map for model predictions by groupings them based on 候选2 ID."""
    pred_metadata_map = {}
...
def _log_image_predictions(experiment, validator, curr_step):
    """Logs predicted boxes for a single 候选2 during training."""
