...
def benchmark(
    ...
    Returns:
        (pandas.DataFrame): 1 pandas DataFrame with benchmark results for each format, including file size, metric,
            and inference time.
    ...
    for i, (name, format, suffix, cpu, gpu) in enumerate(zip(*export_formats().values())):
        ...
        try:
            ...
            key = TASK2METRIC[model.task]  # task to metric, i.e. metrics/mAP50-95(2) for task=detect
...
class ProfileModels:
    ...
    def __init__(
        ...
        Args:
            ...
            imgsz (int): Size of the 候选2 used during profiling.
            half (bool): Flag to indicate whether to use FP16 half-precision for TensorRT profiling.
            trt (bool): Flag to indicate whether to profile using TensorRT.
