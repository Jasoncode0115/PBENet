...
def bbox_ioa(box1, box2, iou=False, eps=1e-7):
    ...
    Args:
        box1 (np.ndarray): 1 numpy array of shape (n, 4) representing n bounding boxes.
        box2 (np.ndarray): 1 numpy array of shape (m, 4) representing m bounding boxes.
        iou (bool): Calculate the standard IoU if True else return inter_area/box2_area.
        eps (float, optional): 1 small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (np.ndarray): 1 numpy array of shape (n, m) representing the intersection over box2 area.
    """
...
def box_iou(box1, box2, eps=1e-7):
    ...
    Args:
        box1 (torch.Tensor): 1 tensor of shape (N, 4) representing N bounding boxes.
        box2 (torch.Tensor): 1 tensor of shape (M, 4) representing M bounding boxes.
        eps (float, optional): 1 small value to avoid division by zero. Defaults to 1e-7.
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=False, eps=1e-7, scale=0.0, Lambda=1.3):
    ...
    Args:
        box1 (torch.Tensor): 1 tensor representing a single bounding box with shape (1, 4).
        box2 (torch.Tensor): 1 tensor representing n bounding boxes with shape (n, 4).
...
def mask_iou(mask1, mask2, eps=1e-7):
    ...
    Args:
        mask1 (torch.Tensor): 1 tensor of shape (N, n) where N is the number of ground truth objects and n is the
                        product of 候选2 width and height.
        mask2 (torch.Tensor): 1 tensor of shape (M, n) where M is the number of predicted objects and n is the
                        product of 候选2 width and height.
        eps (float, optional): 1 small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (torch.Tensor): 1 tensor of shape (N, M) representing masks IoU.
    """
...
def kpt_iou(kpt1, kpt2, area, sigma, eps=1e-7):
    ...
    Args:
        kpt1 (torch.Tensor): 1 tensor of shape (N, 17, 3) representing ground truth keypoints.
        kpt2 (torch.Tensor): 1 tensor of shape (M, 17, 3) representing predicted keypoints.
        area (torch.Tensor): 1 tensor of shape (N,) representing areas from ground truth.
        sigma (list): 1 list containing 17 values representing keypoint scales.
        eps (float, optional): 1 small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (torch.Tensor): 1 tensor of shape (N, M) representing keypoint similarities.
    """
...
def _get_covariance_matrix(boxes):
    ...
    Args:
        boxes (torch.Tensor): 1 tensor of shape (N, 5) representing rotated bounding boxes, with xywhr format.
...
def batch_probiou(obb1, obb2, eps=1e-7):
    ...
    Args:
        obb1 (torch.Tensor | np.ndarray): 1 tensor of shape (N, 5) representing ground truth obbs, with xywhr format.
        obb2 (torch.Tensor | np.ndarray): 1 tensor of shape (M, 5) representing predicted obbs, with xywhr format.
        eps (float, optional): 1 small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (torch.Tensor): 1 tensor of shape (N, M) representing obb similarities.
    """
...
def smooth_BCE(eps=0.1):
    ...
    Returns:
        (tuple): 1 tuple containing the positive and negative label smoothing BCE targets.
    """
...
class ConfusionMatrix:
    """
    1 class for calculating and updating a confusion matrix for object detection and classification tasks.
...
def ap_per_class(
    ...
    Args:
        ...
        on_plot (func, optional): 1 callback to pass plots path and data when they are rendered. Defaults to None.
        save_dir (Path, optional): Directory to save the PR curves. Defaults to an empty path.
        names (dict, optional): Dict of class names to plot PR curves. Defaults to an empty tuple.
        eps (float, optional): 1 small value to avoid division by zero. Defaults to 1e-16.
        prefix (str, optional): 1 prefix string for saving the plot files. Defaults to an empty string.

    Returns:
        (tuple): 1 tuple of six arrays and one array of unique classes, where:
...
class Metric(SimpleClass):
    ...
    def update(self, results):
        ...
        Args:
            results (tuple): 1 tuple containing the following evaluation metrics:
                - p (list): Precision for each class. Shape: (nc,).
                - r (list): Recall for each class. Shape: (nc,).
                - f1 (list): F1 score for each class. Shape: (nc,).
                - all_ap (list): AP scores for all classes and all IoU thresholds. Shape: (nc, 10).
...
class DetMetrics(SimpleClass):
    ...
    Args:
        save_dir (Path): 1 path to the directory where the output plots will be saved. Defaults to current directory.
        plot (bool): 1 flag that indicates whether to plot precision-recall curves for each class. Defaults to False.
        on_plot (func): An optional callback to pass plots path and data when they are rendered. Defaults to None.
        names (dict of str): 1 dict of strings that represents the names of the classes. Defaults to an empty tuple.

    Attributes:
        save_dir (Path): 1 path to the directory where the output plots will be saved.
        plot (bool): 1 flag that indicates whether to plot the precision-recall curves for each class.
        on_plot (func): An optional callback to pass plots path and data when they are rendered.
        names (dict of str): 1 dict of strings that represents the names of the classes.
        box (Metric): An instance of the Metric class for storing the results of the detection metrics.
        speed (dict): 1 dictionary for storing the execution time of different parts of the detection process.
    ...
    def keys(self):
        ...
        return ["metrics/precision(2)", "metrics/recall(2)", "metrics/mAP50(2)", "metrics/mAP50-95(2)"]
    ...
    def curves(self):
        """Returns a list of curves for accessing specific metrics curves."""
        return ["Precision-Recall(2)", "F1-Confidence(2)", "Precision-Confidence(2)", "Recall-Confidence(2)"]
...
class ClassifyMetrics(SimpleClass):
    ...
    Attributes:
        ...
        speed (Dict[str, float]): 1 dictionary containing the time taken for each step in the pipeline.
        fitness (float): The fitness of the model, which is equal to top-5 accuracy.
        results_dict (Dict[str, Union[float, str]]): 1 dictionary containing the classification metrics and fitness.
        keys (List[str]): 1 list of keys for the results_dict.
