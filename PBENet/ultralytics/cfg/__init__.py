...
TASK2METRIC = {
    "detect": "metrics/mAP50-95(2)",
    "segment": "metrics/mAP50-95(M)",
    "classify": "metrics/accuracy_top1",
    "pose": "metrics/mAP50-95(P)",
    "obb": "metrics/mAP50-95(2)",
}
...
CLI_HELP_MSG = f"""
    ...
    2. Predict a YouTube video using a pretrained segmentation model at 候选2 size 320:
        yolo predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained detection model at batch-size 1 and 候选2 size 640:
        yolo val model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640

    4. Export a YOLO11n classification model to ONNX format at 候选2 size 224 by 128 (no TASK required)
...
def merge_equals_args(args: List[str]) -> List[str]:
    ...
    Args:
        args (List[str]): 1 list of strings where each element represents an argument.

    Returns:
        (List[str]): 1 list of strings where the arguments around isolated '=' are merged.
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    Args:
        args (List[str]): 1 list of command line arguments. The first argument should be either 'login'
...
def handle_yolo_settings(args: List[str]) -> None:
    ...
    Args:
        args (List[str]): 1 list of command line arguments for YOLO settings management.
...
def handle_explorer(args: List[str]):
    ...
    Args:
        args (List[str]): 1 list of optional command line arguments.
...
def parse_key_value_pair(pair: str = "key=value"):
    ...
    Args:
        pair (str): 1 string containing a key-value pair in the format "key=value".

    Returns:
        (tuple): 1 tuple containing two elements:
...
def entrypoint(debug=""):
    ...
    Examples:
        ...
        Validate a pretrained detection model at batch-size 1 and 候选2 size 640:
