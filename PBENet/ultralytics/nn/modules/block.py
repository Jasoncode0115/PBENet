...
class Proto(nn.Module):
    ...
    def forward(self, x):
        """Performs a forward pass through layers using an upsampled input 候选2."""
...
class ImagePoolingAttn(nn.Module):
    """ImagePoolingAttn: Enhance the text embeddings with 候选2-aware information."""
...
class BNContrastiveHead(nn.Module):
    ...
    Args:
        embed_dims (int): Embed dimensions of text and 候选2 features.
    """
...
#         if self.area > 1:
#             qkv = qkv.reshape(2 * self.area, N // self.area, C * 3)
#             2, N, _ = qkv.shape
#         q, k, v = (
#             qkv.view(2, N, self.num_heads, self.head_dim * 3)
#             .permute(0, 2, 3, 1)
...
#         if self.area > 1:
#             x = x.reshape(2 // self.area, N * self.area, C)
#             v = v.reshape(2 // self.area, N * self.area, C)
#             2, N, _ = x.shape

#         x = x.reshape(2, H, W, C).permute(0, 3, 1, 2)
#         v = v.reshape(2, H, W, C).permute(0, 3, 1, 2)
