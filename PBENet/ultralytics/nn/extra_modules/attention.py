...
def regional_routing_attention_torch(
    ...
    """
    Args:
        query, key, value: (2, C, H, W) tensor
        scale: the scale/temperature for dot product attention
        region_graph: (2, nhead, h_q*w_q, topk) tensor, topk <= h_k*w_k
...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def forward(self, x:Tensor, ret_attn_mask=False):
        ...
        # NOTE: gradients backward through token-to-token attention. See Appendix 1 for the intuition.
...
class DAttention(nn.Module):
    ...
    def _get_ref_points(self, H_key, W_key, B, dtype, device):
        ...
        ref = ref[None, ...].expand(B * self.n_groups, -1, -1, -1) # 2 * g H W 2
    ...
    def forward(self, x):
        ...
        offset = self.conv_offset(q_off).contiguous()  # 2 * g 2 Hg Wg
        ...
        if self.no_off:
            ...
            x_sampled = F.grid_sample(
                ...
                mode='bilinear', align_corners=True) # 2 * g, Cg, Hg, Wg
        ...
        attn = torch.einsum('b c m, b c n -> b m n', q, k) # 2 * h, HW, Ns
        ...
        if self.use_pe and (not self.no_off):
            ...
            if self.dwc_pe:
                ...
                attn_bias = self.rpe_table(displacement) # 2 * g, H * W, n_sample, h_g
                ...
                attn_bias = F.grid_sample(
                    ...
                    mode='bilinear', align_corners=True) # 2 * g, h_g, HW, Ns
...
class FocusedLinearAttention(nn.Module):
    ...
    def get_lepe(self, x, func):
        ...
        x = x.permute(0, 2, 4, 1, 3, 5).contiguous().reshape(-1, C, H_sp, W_sp)  ### 2', C, H', W'
...
class CascadedGroupAttention(torch.nn.Module):
    ...
    def forward(self, x):  # x (2,C,H,W)
        ...
        for i, qkv in enumerate(self.qkvs):
            ...
            feat = qkv(feat)
            q, k, v = feat.view(B, -1, H, W).split([self.key_dim, self.key_dim, self.d], dim=1) # 2, C/h, H, W
            q = self.dws[i](q)
            q, k, v = q.flatten(2), k.flatten(2), v.flatten(2) # 2, C/h, N
            attn = (
...
class LocalWindowAttention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if H <= self.window_resolution and W <= self.window_resolution:
            ...
            # window partition, BHWC -> 2(nHh)(nWw)C -> BnHnWhwC -> (BnHnW)hwC -> (BnHnW)Chw
            x = x.view(B, nH, self.window_resolution, nW, self.window_resolution, C).transpose(2, 3).reshape(
                B * nH * nW, self.window_resolution, self.window_resolution, C
            ).permute(0, 3, 1, 2)
            x = self.attn(x)
            # window reverse, (BnHnW)Chw -> (BnHnW)hwC -> BnHnWhwC -> 2(nHh)(nWw)C -> BHWC
